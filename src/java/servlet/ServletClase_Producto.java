 ...
 * @author Elvis
 */
public class ServletClase_Producto extends HttpServlet {
    ...
    private void RegistrarClaseProducto(HttpServletRequest request, HttpServletResponse response)
        ...
        boolean resp = Clase_ProductoDAO.insertarClaseProducto(cp);
        if(resp){
    ...
    private void ActualizarClaseProducto(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException{
        Clase_Producto cp = new Clase_Producto();
        cp.setIdClase_Producto(Integer.parseInt(request.getParameter("txtCodigo")));
        cp.setNombreCP(request.getParameter("txtNombre")); 
        
        boolean resp = Clase_ProductoDAO.actualizarClaseProducto(cp);
        if(resp){
            response.sendRedirect("mensaje.jsp?mens='Se ha modificado la clase producto "+cp.getIdClase_Producto()+"'");
        }else{
    ...
    private void EliminarClaseProducto(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException{
        Clase_Producto cp = new Clase_Producto();
        cp.setIdClase_Producto(Integer.parseInt(request.getParameter("idClase_Producto")));
        
        boolean resp = Clase_ProductoDAO.darBajaClaseProducto(cp);
        if(resp){
            response.sendRedirect("mensaje.jsp?mens='Se ha eliminado la clase producto "+cp.getIdClase_Producto()+"'");
        }else{
    ...
    private void RecuperarClaseProducto(HttpServletRequest request, HttpServletResponse response)
        ...
        boolean resp = Clase_ProductoDAO.darAltaClaseProducto(cp);
        if(resp){
            response.sendRedirect("mensaje.jsp?mens='Se ha recuperado la clase producto "+cp.getIdClase_Producto()+"'");
        }else{
